...
die() {
  echo -e "$*" > ~/.cache/toggle_bum.sh.logs
  kitten @ launch --type overlay bash -c "cat ~/.cache/toggle_bum.sh.logs ; echo '[Press any key to exit]' && read -n 1"
  exit 1
}
...
while read -r id cmdline; do
  kitten @ close-window --match id:"$id" || die "Something went wrong while closing current bum instance"
  closed=1
...
if [ -z "$ssh_cmd" ]; then
  kitten @ launch --dont-take-focus --type window --location first --bias 20 bum -toggle || die "Something went wrong while opening local bum instance"
  exit
fi
...
if (( ssh_cmd_lines == 1 )) && [ -n "$ssh_cmd" ]; then
  ...
  host="${host}_bum"
  match="$(grep -riE "Host\s+$host" ~/.ssh/config ~/.ssh/config.d | awk 'NR==1 {print $2}')"
  if [ -z "$match" ]; then
  ...
  kitten @ launch --dont-take-focus --type window --location first --bias 20 ssh "$match" || die "Something went wrong while opening remote bum instance"
else
